...
uv run python Projeto/codigo/10_evaluation.py           # W7 (Grupo A) - **Avaliação estratificada final do v3**. Tabela custo-benefício (11 thresholds × 4 ratios FN:FP) → **threshold canônico = 0,30 (ratio 5:1, max F2=0,783)**. Q6 — 3 faixas operacionais: Verde < 0,145 / Amarelo 0,145-0,30 / Vermelho ≥ 0,30. Estratificação por frota → **L11: modelo emite ZERO alertas em escavadeiras LeTourneau** (45% dos eventos do test, AUC-PR=0,008). Estratificação por tipo, estado pré-evento, unknown vs conhecido. **Insight CM 6.1:** unknown performa ligeiramente melhor (0,89 vs 0,86) — refuta expectativa W5. **Fig 10** matriz de confusão com impacto operacional anotado. → 6 tabelas eval_*.csv + fig10. Tempo: ~30 s
uv run python Projeto/codigo/16_random_forest_comparativo.py # W7 (Grupo A) - **Random Forest tunado para comparação** (Optuna 50 trials, TimeSeriesSplit CV 4 folds, mesma seed=42 do v3). Reforça empiricamente o Diferencial #1 (algoritmo NÃO é o diferencial — metodologia é). → modelos/random_forest_comparativo.joblib + 2 tabelas (rf_metricas, rf_hiperparametros). Tempo: ~30-60 min
uv run python Projeto/codigo/17_distribuicao_antecipacao.py # W7 (Grupo B#2, Qualidade B) - **Distribuição temporal dos TPs do v3**. **Achado crítico:** 50% dos TPs são detecções diretas (antecipação=0); dos 50% restantes, mediana=5,7 min, P75=56 min, P90=146 min. Apenas 18% atingem janela de mobilização (90 min). **L12 nova** — modelo é mais "detector de DG iminente" que "antecipador 4h". → distribuicao_antecipacao.csv + figNeg04 (2 painéis). Tempo: ~15 s
uv run python Projeto/codigo/18_top100_fps_if.py        # W7 (Grupo B#3) - **Top 100 FPs do Isolation Forest** (anomaly_score alto sem DG). **94 dos 100 vêm da PE3797 (escavadeira LeTourneau)**, 99% têm eventos Críticos nas 4h seguintes. **6ª evidência convergente sobre LeTourneau** + insight CM 6.1 + recomendação CM 6.3 (auditoria manual). → top100_fps_if.csv + top100_fps_if_concentracoes.csv + figExH (3 painéis). Tempo: ~30 s
uv run python Projeto/codigo/08d_comparacao_horizontes_cv.py # W7 (Grupo B#1) - **Comparação T2/T4/T8 via TimeSeriesSplit CV** com hiperparâmetros fixos do v3. **Cenário 1 confirmado** (Δ/σ T2 vs T4 = 0,29 < 2σ): T4 mantido canônico. **Achado lateral — fold 4 colapsa em todos os horizontes** (T4 cai de ~0,88 para 0,17) → Insight #12 CM 6.1 (CV agregada mascara colapso). → comparacao_horizontes_cv.csv. Tempo: ~8h (não 6min como estimado)
uv run python Projeto/codigo/19_drift_semanal_junho.py  # W7 (fechamento) - **Drift INTRA-MÊS do v3 em junho** (test). AUC-PR varia de 0,35 (S3 calma) a 0,95 (S4 explosão CA65926) — amplitude de 0,59 pp em 30 dias. **Insight #13 CM 6.1** — monitoramento em produção precisa ser semanal, não mensal. Reforça L4 + L10. → drift_semanal_junho.csv + figExI_drift_semanal_junho.png. Tempo: ~10 s
uv run python Projeto/codigo/20_shap_waterfall_v3.py    # W6 (Fig 12, gerada em W7+) - **SHAP waterfall local** (CM 5.3): explicação de UMA predição individual. Evento CA65933 (caminhão comum, fora do CA65926), p=0,969, DG real. Drivers: qtd_alarmes_muito_alto (+2,94) / razao_alarme (+0,84) / tipo_caminhao (+0,40). Demonstra generalização do v3. → fig12_shap_waterfall_v3.png + shap_waterfall_evento.csv. Requer PYTHONIOENCODING=utf-8. Tempo: ~30 s
uv run python Projeto/codigo/21_target_encoding_comparativo.py # W5 (refinamento, resolvido 06/06) - **target encoding KFold temporal vs frequency encoding** (CM 3.2). Hiperparams fixos do v3, varia só o encoding das 2 categóricas (TAG, operador). **Target encoding PIORA (val −2,25pp, test −0,62pp); critério ganho_val>1pp não atingido → MANTÉM frequency.** Causa: drift temporal + corr negativa freq×taxa_DG. Baseline reproduz v3 exato (test 0,8556). → target_encoding_comparativo.csv. Tempo: ~1 min
uv run python Projeto/codigo/22_v3_estratificado_ca65926.py # W7 (L10, 06/06) - **AUC-PR do v3 com vs sem CA65926** (CM 5.2). Test completo 0,8556 (lift 5,06×); sem CA65926 0,7693 (lift 7,77×); CA65926 só 0,9723 (lift 1,20×, prevalência 80,9%). AUC-ROC quase idêntico (0,9391/0,9368). **Refuta "só detecta um equipamento": generalização genuína, lift MAIOR nos outros 29.** Re-nuança L10. → eval_v3_ca65926.csv. Tempo: ~20 s
uv run python Projeto/codigo/23_antecedencia_vs_acuracia.py # W7 (L12, 07/06) - **Antecipação real vs detecção do instante** (CM 5.2). Alvo redefinido "DG em [t+L,t+4h]" para L=0..120min. **AUC-ROC quase não cai (0,942→0,913 em L=90min), lift estável ~5×** — antecipação de 90-120min EXISTE no score; queda do AUC-PR é só o piso de prevalência. Re-nuança L12 (trade-off precisão×antecedência, gerenciável por limiar). → antecedencia_vs_acuracia.csv + figExJ. Tempo: ~30 s
```
