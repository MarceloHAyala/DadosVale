...
No limiar 0,5, a análise temporal dos TPs revela: **50% são detecções diretas** (antecipação=0); dos restantes, mediana = 5,7 min, e só 18% atingem 90 min. **Refinamento de 07/06:** redefinindo o alvo como "DG em [t+L, t+4h]" e variando L, a **AUC-ROC quase não cai** (0,942 em L=0 → 0,913 em L=90min) e o **lift fica estável em ~5×** — a queda do AUC-PR só acompanha a queda da prevalência (piso da métrica). **Conclusão:** a antecipação de 90-120 min existe no score; é realizável reduzindo o limiar (faixa Amarela do Q6, 0,145). O limite real é o trade-off precisão × antecedência, gerenciável por ponto de operação, não incapacidade de antecipar. Mitigação CM 6.3: operação em dois níveis (Vermelho iminente / Amarelo antecipado), target 8-12h, ou combinação com Weibull AFT. Figura Extra J.
...
- [ ] **T1.2 Reconciliar L12 (antecipação) com as horas evitáveis (figNeg03), com a nuance de 07/06.** A L12 foi re-nuançada: no limiar 0,5 só 18% dão 90 min, MAS a capacidade de antecipar 90-120 min existe no score (AUC-ROC 0,913 em L=90min, lift ~5× estável, Figura Extra J). A narrativa honesta e favorável: "a Frente 1 antecipa de fato (AUC-ROC ~0,91 a 90 min); realizar essa antecipação é escolher o ponto de operação, e a faixa Amarela do Q6 (0,145) é esse mecanismo, ao custo de mais falsos positivos". Ainda assim, usar o cenário **conservador** de horas evitáveis como manchete e declarar o trade-off precisão × antecedência abertamente.
