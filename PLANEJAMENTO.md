...
- [ ] **`08e_lightgbm_v2_no_cascade.py` — Variante v3 sem `horas_desde_ultimo_DG`** (em execução no momento). Mesma config do v2 (Optuna 50 trials + TimeSeriesSplit CV 4 folds + determinismo), apenas com a feature problemática removida (34 features em vez de 35). **Objetivo:** quantificar trade-off "cascade vs primeiro DG" via comparação estratificada em 3 subgrupos (geral / primeiro_DG / cascata). Decisão Opção D (v2 e v3 paralelos canônicos) condicional ao resultado.
