...
- [ ] **[Determinismo estrito em v2 — derivado de discussão em W5, 24/05/2026]** Adicionar `deterministic=True` e `force_col_wise=True` aos `LGBM_PARAMS_BASE` do `08b_lightgbm_v2.py` (ou equivalente). **Justificativa:** v1 atual usa `n_jobs=-1` (paralelismo total) sem `deterministic`, então execuções múltiplas podem variar em ~0,001 AUC-PR por *thread scheduling* — aceitável em v1 (modelo de referência rápido para o GATE), mas **não aceitável em v2** que é o resultado canônico que vai para o relatório final. Custo: tempo de treino ~15-30% maior (estimado de ~17s atuais para ~22s; insignificante na escala de Optuna com 50 trials que já leva minutos). Benefício: dois runs do v2 produzem AUC-PR / Recall / Precision idênticos até a última casa decimal — reprodutibilidade *bit-exact*, requisito para qualquer auditoria do trabalho. Registrar a decisão em `controle_alteracoes.md` quando aplicado.
