...
- [ ] **`Projeto/codigo/08c_comparacao_horizontes_cv.py` — Profundidade 1 rigorosa via TimeSeriesSplit CV** (agendado em 24/05 após v2 mostrar T2 vs T4 indistinguíveis em single-fold). Treinar 3 modelos v2 (T2 / T4 / T8) com hiperparâmetros idênticos sobre **TimeSeriesSplit CV de 4 folds expandidos** (mesma estrutura da Mitigação 1), reportar AUC-PR média ± desvio padrão de cada horizonte. **Custo:** ~1-2h (3 × 4 = 12 treinos curtos, cada ~30s). **Saída:** `relatorio/tabelas/comparacao_horizontes_cv.csv` (3 horizontes × média + std) + 1 parágrafo de análise.
