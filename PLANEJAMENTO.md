...
- [X] Salvar `Projeto/dados/features/v2.parquet` — feito na sessão anterior (29 features + 3 targets). `v2_split.parquet` foi o sucessor canônico ao final de W4 (mesmas linhas + col `split`); v2.parquet preservado como matriz "pré-split" para compatibilidade. **Nota cross-week:** em 22/05 (W5), `v2_split.parquet` foi por sua vez substituído por `v3.parquet` como *input* canônico de modelagem (mesma *shape* mas com `tag_freq` e `operador_freq` recomputadas sobre treino apenas, via `06b_fix_encoding_leakage.py` — ver entrada `controle_alteracoes.md` 2026-05-22).
