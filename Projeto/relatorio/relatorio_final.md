...
Esse comportamento em regimes tem consequência direta: o conjunto de teste (junho) é dominado por uma anomalia mecânica localizada, um fato a levar em conta ao interpretar o desempenho do modelo, e discutido em profundidade na Seção 5.
...
Essa passagem, da v2 para a v3, é uma das lições metodológicas centrais do trabalho: um AUC-PR alto pode esconder uma estratégia operacionalmente fraca, e só a interpretabilidade (SHAP) somada a uma investigação dirigida revela isso. Sem esse passo, teria sido entregue um modelo com bom número e comportamento ruim.
...
São propostas extensões concretas, ordenadas por retorno esperado:
