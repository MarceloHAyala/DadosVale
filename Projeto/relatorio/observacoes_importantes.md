...
- **W7 Grupo B (01/06/2026 — Análises complementares):** ver `PLANEJAMENTO.md` → seção W7
  - **B#2 — Tempo de antecipação (Qualidade B):** no limiar 0,5, **50% dos TPs são detecções diretas** (antecipação=0) e só 18% atingem 90 min. **Mas (refinamento de 07/06, `23_antecedencia_vs_acuracia.py`):** redefinindo o alvo como "DG em [t+L, t+4h]", a AUC-ROC do v3 quase não cai (0,942 em L=0 → 0,913 em L=90min) e o lift fica estável em ~5×. A antecipação de 90-120 min EXISTE no score; ela é realizável reduzindo o limiar (faixa Amarela do Q6, 0,145). **L12 re-nuançada:** o limite real é o trade-off precisão × antecedência, gerenciável por ponto de operação, não incapacidade de antecipar. Figura Extra J. Mitigações em CM 6.3: operação em dois níveis (Vermelho iminente / Amarelo antecipado), target 8-12h, Frente 2 (Weibull AFT).
