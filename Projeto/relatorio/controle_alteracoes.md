...
> ⚠️ **CORREÇÃO no mesmo dia (ver entrada seguinte):** o número 0,913 abaixo era do alvo INCLUSIVO, inflado por acerto via um DG mais próximo na janela. A medição estrita correta deu AUC-ROC 0,82. Esta entrada fica preservada como registro do raciocínio intermediário; a conclusão honesta está na entrada "2026-06-07 (correção) — Antecipação estrita".
...
- **Impacto:** L12 reescrita em rascunho.md, observacoes_importantes.md e PLANEJAMENTO.md de "incapacidade de antecipar" para "trade-off precisão × antecedência, gerenciável por ponto de operação". Nova **Figura Extra J** (`figExJ_antecedencia_vs_acuracia.png`) promovida como candidata ao corpo do relatório (CM 5.2). Saídas: `antecedencia_vs_acuracia.csv` + figura.

---

### 2026-06-07 (correção) — Antecipação estrita: o 0,91 era inflado; o honesto é 0,82 (CM 5.2 / CM 6.2)

- **ANTES:** a entrada anterior (mesmo dia) concluiu, via `23_antecedencia_vs_acuracia.py`, que a AUC-ROC se mantinha em 0,913 em L=90min, lendo isso como antecipação forte. O alvo usado era **inclusivo**: "existe ALGUM DG em [t+L, t+4h]". Esse alvo aceita eventos que já têm um DG mais próximo (dentro de L); nesses, o modelo dispara pelo DG iminente, não por prever o distante. O 0,913 estava, portanto, **inflado por acerto via DG mais próximo** (terminologia: NÃO é "cascata" no sentido estrito, nem é a "acumulação" de alarmes que gera um DG; é o agrupamento temporal de DGs já rotulados).
- **DEPOIS:** `25_antecipacao_estrita.py` e `26_figura_antecipacao_honesta.py` (07/06) refizeram com alvo **estrito**: positivo só se o PRÓXIMO DG estiver entre L e 4h, **excluindo** eventos com qualquer DG antes de L (os iminentes). **Resultado honesto:** AUC-ROC = 0,828 (L=30), 0,814 (L=60), **0,816 (L=90)**, 0,818 (L=120); lift ~5×; o vão para o inclusivo (~0,10) é a contaminação. No ponto de operação 0,30, a antecipação pura de 90min tem recall 43% e precisão 16% (`24_limiar_para_antecipacao.py`).
- **Justificativa:** rigor metodológico. A capacidade de antecipação é **genuína** (0,82 está bem acima de 0,5 aleatório), porém **modesta**, não os 0,91 sugeridos. A distinção entre as duas medições (inclusiva vs estrita) é o cerne da honestidade do achado.
- **Impacto:** L12 reescrita com o texto aprovado pelo usuário (07/06) usando 0,82, em rascunho.md, observacoes_importantes.md e PLANEJAMENTO.md (incluindo a sugestão T1.2). **Figura canônica passa a ser a `figExK_antecipacao_honesta.png`** (inclusivo vs estrito lado a lado); a figExJ fica como artefato preliminar superado. Scripts 24/25/26 registrados no README e auditoria_figuras.md. A conclusão final sobre o modelo: bom classificador de risco de 4h (AUC-PR 0,77-0,86) + medidor real de deterioração de horas (0,82 estrito) + antecipador de longo prazo modesto; deterioração de escala semanal coberta pela Frente 2 (Weibull).
