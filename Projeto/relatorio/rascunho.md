...
#### Antecipação real vs detecção do instante (Figura Extra J)

Para separar "o que o modelo consegue" de "o que dispara no limiar 0,5", redefinimos o alvo como "existe DG em [t+L, t+4h]" para tempos mínimos de antecedência L crescentes, mantendo o mesmo score do v3, e medimos a degradação (`23_antecedencia_vs_acuracia.py`).

[Figura Extra J — Antecipação real vs detecção do instante](figuras/figExJ_antecedencia_vs_acuracia.png)

| L (antecedência mínima) | prevalência | AUC-PR | lift | AUC-ROC |
|---:|---:|---:|---:|---:|
| 0 (sanity = target original) | 0,173 | 0,867 | 5,00× | 0,942 |
| 30 min | 0,149 | 0,727 | 4,88× | 0,924 |
| 60 min | 0,136 | 0,660 | 4,84× | 0,916 |
| **90 min** | 0,125 | 0,617 | 4,95× | **0,913** |
| 120 min | 0,111 | 0,571 | 5,13× | 0,910 |

**Achado central:** a AUC-ROC (independente de prevalência) quase não cai, mantendo-se em **0,91 mesmo exigindo 90-120 min de antecedência real**, e o **lift fica estável em ~5×**. A queda da AUC-PR acompanha a queda da prevalência (o piso da métrica), não uma perda de habilidade. Sanity check: L=0 reproduz o alvo original (prevalência 0,173, AUC-PR 0,867). Sem o CA65926, a AUC-ROC em L=90min ainda é 0,90 (lift ~6×), ou seja, a antecipação generaliza.

**Conclusão (re-nuança da L12):** a antecipação de até 90-120 min **existe no modelo**, está no ranqueamento do score. O que a análise de 18%@90min (acima) mostrava era o comportamento no limiar 0,5, onde os alertas antecipados ficam logo abaixo do corte. Realizar a antecipação é uma escolha de ponto de operação: a **faixa Amarela do Q6 (limiar 0,145)** captura justamente esses alertas precoces, ao custo de menor precisão. **Registrada como limitação L12 (re-nuançada) em CM 6.2:** o limite real é o trade-off precisão × antecedência, gerenciável por limiar, não uma incapacidade de antecipar. Mitigações em CM 6.3: operação em dois níveis (Vermelho iminente / Amarelo antecipado), variante com target 8-12h, combinação com a Frente 2 (Weibull AFT).
...
**L12 — Trade-off antecedência × precisão: no limiar 0,5 os alertas se concentram perto do DG, mas a capacidade de antecipação existe no score (RE-NUANÇADA em 07/06):**
Identificada via `17_distribuicao_antecipacao.py` (W7 Grupo B, 01/06) e aprofundada via `23_antecedencia_vs_acuracia.py` (07/06). **Fato 1 (operando no limiar 0,5):** dos 9.821 verdadeiros positivos, **50,4% são "detecções diretas"** (o próprio evento já é um DG, antecipação = 0); entre as antecipações reais, mediana = 5,7 min, P90 = 146 min, e apenas 18% chegam aos 90 min de mobilização. **Fato 2 (capacidade do score, independente de limiar):** redefinindo o alvo como "existe DG em [t+L, t+4h]" e variando o tempo mínimo de antecedência L, a **AUC-ROC do v3 quase não cai** (0,942 em L=0 → 0,913 em L=90min → 0,910 em L=120min) e o **lift fica estável em ~5×**. A AUC-PR cai (0,867 → 0,617 em L=90min), mas acompanhando a queda da prevalência (o piso da métrica), não por perda de habilidade. **Reconciliação:** um evento 90 min antes do DG recebe nota alta, porém logo abaixo de 0,5, então não dispara nesse limiar (vira falso negativo no Fato 1), mas continua bem ranqueado acima dos eventos sem DG (Fato 2). **A antecipação de até 90-120 min existe no modelo; ela é realizável reduzindo o limiar.** **Magnitude:** moderada e gerenciável por ponto de operação (não é incapacidade de antecipar). **Implicação operacional:** a faixa Amarela do Q6 (limiar 0,145) é precisamente o mecanismo de alerta antecipado, ao custo de menor precisão (mais falsos positivos, baratos frente a um DG perdido). **Mitigações (CM 6.3):** (a) operar a Frente 1 em dois níveis (Vermelho 0,30 para DG iminente, Amarelo 0,145 para antecipação); (b) variante com target mais longo (8-12h); (c) combinar com a Frente 2 (Weibull AFT, multi-horizonte). **Material para CM 5.2 (Figura Extra J) e CM 6.3.** Figuras `figNeg04_distribuicao_antecipacao.png` (Fato 1) e `figExJ_antecedencia_vs_acuracia.png` (Fato 2) documentam o achado.
