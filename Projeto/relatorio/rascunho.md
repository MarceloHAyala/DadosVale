...
**Sobre o uso operacional dos achados (importante):** durante a análise, a análise distinguiu dois fatos que à primeira vista pareciam um só. O CA65926 (responsável por 24,7% dos DGs do semestre) é o equipamento mais saliente do teste, e o sinal estrutural não-supervisionado (Isolation Forest) depende fortemente dele. Mas o modelo de classificação supervisionado generaliza: sem o CA65926 a AUC-PR cai apenas de 0,8556 para 0,7693 (com lift sobre o acaso até maior nos demais equipamentos) e a AUC-ROC fica praticamente intacta. O número absoluto é parcialmente inflado pela altíssima prevalência do CA65926, não por dependência dele. Ainda assim, reposicionamos a entrega por uma razão de cobertura: o modelo de classificação é uma das ferramentas do conjunto, mas o **valor operacional principal está no monitoramento estratificado por equipamento + decisão de manutenção informada pelas duas frentes em conjunto (LightGBM v3 + Weibull AFT)** — não num único *score* automático. Essa nuance permeia toda a seção de Resultados.
...
**Limitação reconhecida desde o entendimento do negócio, com nuance importante (re-nuançada em 06/06).** A análise empírica posterior (Resultados + Limitação L10) mostra que o número absoluto de AUC-PR em junho (0,8556) é parcialmente inflado pela altíssima prevalência do CA65926, mas que a Frente 1 supervisionada **generaliza**: sem o CA65926 a AUC-PR é 0,7693, com AUC-ROC praticamente intacta (0,9368) e lift sobre o acaso até maior nos demais 29 equipamentos. A operação integrada das duas frentes é recomendada por **cobertura**, não por fragilidade do classificador: a Frente 1 não cobre bem as escavadeiras LeTourneau (L11, AUC-PR 0,008), e a Frente 2 (visão estrutural por equipamento + política de manutenção informada por sobrevivência), somada ao diagnóstico do Isolation Forest, cobre justamente o regime que a Frente 1 não alcança.
...
**Interpretação inicial (incompleta):** no limiar 0,5, o v3 funciona muito mais como detector de DG iminente do que como antecipador de 4h. Mas essa leitura olha apenas o ponto de operação 0,5; ela não diz se a *capacidade* de antecipar existe no score.

#### Antecipação real vs detecção do instante (Figura Extra K)

Para separar "o que o modelo consegue" de "o que dispara no limiar 0,5", redefinimos o alvo exigindo antecedência mínima L crescente, mantendo o mesmo score do v3, e medimos a AUC-ROC (`23_antecedencia_vs_acuracia.py`, `25_antecipacao_estrita.py`, `26_figura_antecipacao_honesta.py`). É preciso medir com rigor, porque a forma ingênua de redefinir o alvo infla o resultado.

[Figura Extra K — Antecipação real do v3: pura vs inflada por DG mais próximo](figuras/figExK_antecipacao_honesta.png)

| L | AUC-ROC inclusivo (permite DG mais próximo) | AUC-ROC estrito (antecipação pura) | lift estrito |
|---:|---:|---:|---:|
| 0 | 0,942 | 0,942 | 5,00× |
| 30 min | 0,924 | 0,828 | 5,86× |
| 60 min | 0,916 | 0,814 | 5,18× |
| **90 min** | 0,913 | **0,816** | 4,95× |
| 120 min | 0,910 | 0,818 | 4,91× |

**A medição ingênua engana, e isso precisa ser dito.** O alvo *inclusivo* ("existe algum DG em [t+L, t+4h]") aceita eventos que já têm um DG mais próximo na janela; nesses, o modelo dispara pelo evento iminente, não por prever o distante, e o resultado sobe artificialmente para AUC-ROC 0,91 em L=90 min. O alvo *estrito* (próximo DG entre L e 4h, **sem nada iminente antes**) remove esse acerto por DG mais próximo. O vão entre as duas curvas (laranja na figura) é exatamente essa contaminação.

**Achado honesto:** no recorte estrito, a AUC-ROC fica em **~0,82 de 30 a 120 min** e o **lift se mantém em ~5×**. Bem acima do aleatório (0,5), portanto há **capacidade de antecipação genuína**, ainda que mais modesta do que a métrica inclusiva sugeria. Sanity check: em L=0 as duas curvas coincidem em 0,942 (sem exclusão, sem contaminação).

**Conclusão (alimenta a L12 em CM 6.2):** a antecipação de 90-120 min existe no score, mas é modesta; no limiar 0,5 os alertas se concentram perto do DG (Camada 1), enquanto a capacidade de ranquear avisos precoces (Camada 2) só se realiza num ponto de operação mais baixo, a faixa Amarela do Q6 (0,145), ao custo de menor precisão. O limite real é o trade-off precisão × antecedência, gerenciável por corte, não incapacidade de antecipar.
...
**L12 — Trade-off antecedência × precisão: o v3 antecipa de forma modesta porém real; é mais detector de risco iminente que antecipador de longo prazo.**

Avaliada em três camadas (`17_distribuicao_antecipacao.py`, `23_antecedencia_vs_acuracia.py`, `25_antecipacao_estrita.py`).

**Camada 1, no ponto de operação 0,5.** Dos verdadeiros positivos, 50,4% são detecções diretas (o próprio evento já é um DG, antecipação zero); entre as antecipações reais, a mediana é 5,7 min e apenas 18% atingem os 90 min da janela de mobilização. Nesse corte, o v3 funciona predominantemente como detector de DG iminente.

**Camada 2, capacidade do score (independente de corte).** Redefinindo o alvo como "existe DG em [t+L, t+4h]" e exigindo antecedência mínima L crescente, é preciso medir com rigor: a versão inclusiva (que aceita um DG mais próximo na janela) chega a AUC-ROC 0,91 em L=90 min, mas esse número é inflado por acerto via um DG mais próximo, o modelo dispara pelo evento iminente, não por prever o distante. No recorte estrito (próximo DG entre 90 min e 4h, sem nada iminente antes), a AUC-ROC honesta é 0,82 e o lift se mantém em ~5× de 30 a 120 min (Figura Extra K). Ou seja, há capacidade de antecipação genuína, ainda que mais modesta que a métrica inclusiva sugeria.

**Reconciliação.** Um evento 90 min antes de um DG recebe nota alta, porém logo abaixo de 0,5, então não dispara nesse corte (vira falso negativo na Camada 1), mas continua bem ranqueado acima dos eventos sem DG (Camada 2). A antecipação existe no score; realizá-la é uma escolha de ponto de operação.

**Realização operacional e custo.** No ponto de operação 0,30 (Vermelho do Q6), a antecipação pura de 90 min é capturada com recall de ~43% e precisão de ~16%; baixar para 0,145 (Amarelo) aumenta o recall ao custo de mais alarmes falsos. O trade-off precisão × antecedência é o limite real, gerenciável por corte, não uma incapacidade de antecipar.

**Magnitude:** moderada e bem caracterizada. **Mitigações (CM 6.3):** (a) operar a Frente 1 em dois níveis (Vermelho 0,30 para risco iminente, Amarelo 0,145 para antecipação); (b) variante com target mais longo (8-12h); (c) combinar com a Frente 2 (Weibull AFT, multi-horizonte), que cobre a deterioração de escala semanal que o modelo de 4h não alcança. **Figuras:** `figNeg04` (Camada 1) e `figExK` (Camada 2).
