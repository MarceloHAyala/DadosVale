...
A matriz canônica de modelagem após a etapa de correção do *leakage* subtil de *frequency encoding* (W5, executada em 22/05/2026 — ver `controle_alteracoes.md` entrada de 2026-05-22 e `notas_metodologicas.md` Seção 2) é o arquivo `Projeto/dados/features/v3.parquet`, com **544.885 linhas × 52 colunas** e **14,9 MB**, produto da execução sequencial de `05_features.py` (29 features + 3 targets), `06_split.py` (coluna `split`) e `06b_fix_encoding_leakage.py` (recomputação de `tag_freq` e `operador_freq` sobre o *split* de treino apenas). A composição completa das 52 colunas é a seguinte:
...
| Features avançadas (Famílias 1-7) | 24 | `05_features.py` etapas 4-10 — W4 (`tag_freq` e `operador_freq` recomputadas em W5 via `06b_fix_encoding_leakage.py`) |
...
- **`v2_parcial.parquet`** (19,6 MB) — 19 *features* das Famílias 0 a 4 (W4 parcial, antes das Famílias 5-7).
- **`v2.parquet`** (22,4 MB) — 29 *features* completas + 3 *targets* multi-janela (W4 completo, antes do *split* temporal).
- **`v2_split.parquet`** (14,9 MB) — `v2.parquet` + coluna `split` (W4 final, antes do *fix* de *leakage* do *encoding*; preservado como referência histórica, **não usar em modelagem**).
- **`v3.parquet`** (14,9 MB) — `v2_split.parquet` com `tag_freq` e `operador_freq` recomputadas sobre treino apenas (W5, **input canônico de toda a Modelagem**).
...
O estado canônico do *pipeline* **após a conclusão de `06_split.py` (W4)** é a matriz `v2_split.parquet` (544.885 linhas × 52 colunas, **14,9 MB**) — `v2.parquet` original acrescido da coluna `split ∈ {train, val, test}`. Essa matriz foi a referência intermediária para o *fix* de *leakage* de *encoding* aplicado em seguida (`06b_fix_encoding_leakage.py`, W5, 22/05/2026), produzindo a matriz canônica final `v3.parquet` (mesma *shape*, com `tag_freq` e `operador_freq` recomputadas sobre treino apenas) que vira o *input* único para toda a fase de Modelagem em W5+. *Scripts* a jusante lerão `v3.parquet` e filtrarão pela coluna `split` nos pontos de treino, *tuning* e avaliação; `v2.parquet` e `v2_split.parquet` ficam preservados como referências históricas de auditoria mas não devem ser usados diretamente em modelagem, evitando contornar o protocolo de avaliação temporal ou reintroduzir o *leakage* corrigido.
